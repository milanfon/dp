    ...
    Maximize Browser Window\end{code}
        ...
        \begin{figure}
            \begin{tikzpicture}[node distance=2cm, auto]
                    \node (start) [block, fill=red!30] {Record and tag};
                    \node (in1) [block, below of=start, fill=blue!30] {Create test};
                    \node (pro1) [block, below of=in1, fill=gray!50] {Edit prompt};
                    \node (pro2) [block, below of=pro1, fill=gray!50] {Generate};
                    \node (out1) [block, below of=pro2, fill=gray!50] {View};
                    \node (pro3) [block, below of=out1, fill=gray!50] {Manual Edit};
                    \node (dec1) [block, below of=pro3, fil=orange] {Validate test};
                    \node (pro4) [block, below of=dec1, fill=blue!30] {Deploy servers and run tests};
                    \node (stop) [block, below of=pro4, fill=green!50] {Evaluate results};
