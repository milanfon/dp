...
\newacronym{nlp}{NLP}{Natural Language Processing - Zpracování přirozeného jazyka}
\newacronym{ir}{IR}{Information Retrieval - Získávání informací}
...
\newglossaryentry{topp}{
    name={Top P},
    description={Hyperparametr ovládající rozmanitost generovaného textu. Dynamicky vybírá, jak velkou část pravděpodobnostní distribuce využít. Jeho hodnota údává velikost použité kumulativní pravděpodobnosti v procentech. Při snížení pod 100\% jsou méně pravděpodobná slova ignorována.}
 }
...
\newglossaryentry{rag}{
    name={RAG},
    description={Retrieval-Augmented Generation (RAG) je metoda pro vylepšení generativních modelů získáním relevantních informací z velké datové sady a jejím využitím v kontextu pro generování přesnějších a relevantnějších výsledků.}
}

\newglossaryentry{assert}{
    name={assert},
    description={Příkaz používaný k ověření, zda je výsledek testu v souladu s očekávanými hodnotami, což umožňuje automatizovaně identifikovat chyby v testovaném kódu.}
}
