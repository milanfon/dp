*** Settings ***
Library           SeleniumLibrary

*** Variables ***
${BROWSER}        Chrome
${URL}            http://localhost:4680/tbuis/
${USERNAME}       green
${PASSWORD}       pass
${TEACHER_USERNAME}       strict
${TEACHER_PASSWORD}       pass

*** Test Cases ***
Student Test
    Open Browser    ${URL}    ${BROWSER}
    Maximize Browser Window
    Login As Student
    Click Element   xpath://a[@id='stu.menu.otherExamDates']
    Click Element   xpath://button[@id='stu.otherExamDates.table.registerButton-2-0']
    Wait Until Page Contains Element   xpath://*[@id='stu.otherExamDates.successAlert']   10
    Click Element   xpath://a[@id='stu.menu.myExamDates']
    Page Should Contain Element   xpath://td[contains(text(),'Programming in Java')]
    Close Browser

Teacher Test
    Open Browser    ${URL}    ${BROWSER}
    Maximize Browser Window
    Login As Teacher
    Click Element   xpath://a[@id='tea.menu.myExamDates']
    Click Element   xpath://button[@id='tea.myExamDates.table.participantsButton-1-0']
    Page Should Contain Element   xpath://td[contains(text(),'Mia Orange')]
    Close Browser

*** Keywords ***
Login As Student
    Input Text    xpath://input[@id='loginPage.userNameInput']    ${USERNAME}
    Input Text    xpath://input[@id='loginPage.passwordInput']    ${PASSWORD}
    Click Element   xpath://button[@id='loginPage.loginFormSubmit']

Login As Teacher
    Input Text    xpath://input[@id='loginPage.userNameInput']    ${TEACHER_USERNAME}
    Input Text    xpath://input[@id='loginPage.passwordInput']    ${TEACHER_PASSWORD}
    Click Element   xpath://button[@id='loginPage.loginFormSubmit']